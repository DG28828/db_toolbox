...
for k = 1:numel(Methods)
    ...
    nc_data.directional.(method).f = ...
        burst_results(1).out_dirspectrum.(method).f;

    nc_data.directional.(method).theta = ...
        burst_results(1).out_dirspectrum.(method).theta;

    nFreq = numel(nc_data.directional.(method).f);
    nTheta = numel(nc_data.directional.(method).theta);

    % Inicializar espectros direccionales
    nc_data.directional.(method).E = NaN(nFreq, nTheta, nBursts);
