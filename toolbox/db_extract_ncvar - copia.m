...
out.byCampaign  = struct('site', {}, ...
                         ...
                         'output_dimensions', {}, ...
                         'attributes', {});
out.burst_index = zeros(0,1,'uint32');
out.table       = table();
    ...
    for c = 1:numel(camps_site)
        ...
        try
            ...
            if has_burst
                ...
                if local_is_all_burst(opts.Burst)
                    burst_index = uint32((1:nburst_selected).');
                else
                    burst_index = uint32(opts.Burst(:));
                end
            
            else
                burst_index = zeros(0,1,'uint32');
            end
            ...
            out.byCampaign(k).burst_index = burst_index;
            ...
            if has_burst && opts.Concat && can_concat
                ...
                if ~isempty(time)
                    ...
                    out.burst_index = [out.burst_index; burst_index(:)];
